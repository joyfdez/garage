...
export async function generateMetadata({
  ...
  return { title: `People @${username} follows — Garage` };
}
  ...
  return (
    <div className="bg-paper min-h-dvh pb-24 page-enter">
      <div className="px-5 pt-safe-page-8 pb-6">
        ...
        <h1 className="font-display font-extrabold text-xl text-ink">Following</h1>
        <p className="text-ink-muted text-sm mt-0.5">
          @{username} follows {ordered.length} {ordered.length === 1 ? "person" : "people"}
        </p>
      </div>
      <div className="px-5">
        {ordered.length === 0 ? (
          <div className="py-16 text-center">
            <p className="text-ink-muted text-sm">Not following anyone yet.</p>
            <Link
              href="/people"
              className="text-xs text-green-bright hover:underline mt-2 inline-block"
            >
              Find people →
            </Link>
          </div>
